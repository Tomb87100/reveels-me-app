...
class ReveelsMeV3SupabaseUser extends BaseAuthUser {
  ReveelsMeV3SupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> reveelsMeV3SupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = ReveelsMeV3SupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
